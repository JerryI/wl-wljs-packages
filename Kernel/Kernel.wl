...
WLJS`PM`Packages /: Keys[WLJS`PM`Packages] := $name2key // Keys
WLJS`PM`Packages[name_String, fields__] := $packages[ $name2key[name], fields ] 
WLJS`PM`Packages[name_String] := $packages[ $name2key[name] ] 

WLJS`PM`Packages /: KeyExistsQ[ WLJS`PM`Packages[name_String, fields__] ] := !MissingQ[ $packages[ $name2key[name], fields ] ]
WLJS`PM`Packages /: KeyExistsQ[ WLJS`PM`Packages[name_String] ] := KeyExistsQ[ $packages,  $name2key[name]  ]

WLJS`PM`Packages /: Set[ WLJS`PM`Packages[name_String, fields__], value_ ] := With[{tag = $name2key[name]},
...
WLJS`PM`SaveConfiguration := With[{},
...
CheckUpdates[a_Association, Rule[Github, _]] := Module[{package, new, now},
  (* fetch any *)
  package = FetchInfo[a];
  If[!AssociationQ[package], Echo["WLJS::PM >> cannot check the github! skipping..."]; Return[False, Module]];
...
WLJS`PM`InstallByURL[url_String, cbk_:Null] := Module[{remote},
